class Trip < ApplicationRecord
  ...
  scope :origin_in, -> (geom, distance) do
    where(id: joins(:origin).where("ST_DWithin(waypoints.geom::geography, ST_GeomFromText(?, 4326)::geography, ?)", geom.as_text, distance).pluck(:id))
  end

  # Adjusted `destination_in` scope to use `ST_DWithin`
  scope :destination_in, -> (geom, distance) do
    where(id: joins(:destination).where("ST_DWithin(waypoints.geom::geography, ST_GeomFromText(?, 4326)::geography, ?)", geom.as_text, distance).pluck(:id))
  end
