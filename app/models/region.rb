class Region < ApplicationRecord
  ...
  ### SCOPES ###
  scope :containing, -> (geom2) { where("ST_Contains(simplified_geom, ?)", geom2) }
  scope :containing_point, -> (lng, lat) { where("ST_Contains(simplified_geom, ST_SetSRID(ST_Point(?, ?), 4326))", lng, lat) }
