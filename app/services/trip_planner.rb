...
class TripPlanner
  ...
  def filter_itineraries
    ...
    itineraries = @trip.itineraries.map do |itin|
      ...
      if @trip.trip_type.exclude?('walk') && itin.trip_type == 'transit' && itin.legs.any? { |leg| leg['mode'] != 'WALK' } && itin.legs.detect { |leg| leg['mode'] == 'WALK' && leg["distance"] > max_walk_distance }
        next
      end      
